...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, input_word_embeddings=None):
        ...
        if input_word_embeddings is not None:
            seq_length = input_word_embeddings.size(1)
            position_ids = torch.arange(seq_length, dtype=torch.long, device=input_word_embeddings.device)
            position_ids = position_ids.unsqueeze(0).expand_as(input_word_embeddings[:,:,0])
            if token_type_ids is None:
                token_type_ids = torch.zeros_like(input_word_embeddings[:,:,0])

            words_embeddings = input_word_embeddings
            position_embeddings = self.position_embeddings(position_ids)
            token_type_embeddings = self.token_type_embeddings(token_type_ids)
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, input_word_embeddings=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None):
        if attention_mask is None:
            if input_ids is not None:
                attention_mask = torch.ones_like(input_ids)
            elif input_word_embeddings is not None:
                attention_mask = torch.ones_like(input_word_embeddings[:,:,0])
        if token_type_ids is None:
            if input_ids is not None:
                token_type_ids = torch.zeros_like(input_ids)
            elif input_word_embeddings is not None:
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, input_word_embeddings)
...
class LXRTPretraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, input_word_embeddings=None, attention_mask=None, masked_lm_labels=None,
                visual_feats=None, pos=None, obj_labels=None, matched_label=None, ans=None):
        (lang_output, visn_output), pooled_output = self.bert(
            input_ids, token_type_ids, input_word_embeddings, attention_mask,
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, input_word_embeddings=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None):
        feat_seq, pooled_output = self.bert(input_ids, token_type_ids, input_word_embeddings, attention_mask,
