...
This README and a large part of the image captioning code is adopted from [https://github.com/ruotianluo/self-critical.pytorch].

To generate augmented data, use:
```
  python tools/hal_data.py 
```

To train a image captioning model, use:
```
  python tools/train.py --cfg configs/transformer/transformer.yml --id transformers
```

Where ```transformer.yml``` use traditional MLE for training. ```transformer_mle.yml```, ```transformer_icmle.yml``` and ```transformer_ecmle.yml``` use augmented data with MLE, Implicit CMLE and Explicit CMLE respectively.

To do full evaluation, use:
```
  python tools/eval.py \
    ...
    --eval_out_dir /out
```
If use data augmentation, add the --use_hal option. Below are detailed instructions of how to setup this image captioning codebase adopted from [https://github.com/ruotianluo/self-critical.pytorch]. The data we used in the paper is COCO.
