...
def eval_split(model, crit, loader, CHAIRevaluator, eval_kwargs={}):
    ...
    use_hal = eval_kwargs.get('use_hal', False)
    model_id = eval_kwargs['id']
    ...
    while True:
        ...
        if beam_size > 1 and verbose_beam:
            for i in range(fc_feats.shape[0]):
                print('\n'.join([utils.decode_sequence(model.vocab, _['seq'].unsqueeze(0), model.opt)[0] for _ in model.done_beams[i]]))
                print('--' * 10)
        sents = utils.decode_sequence(model.vocab, seq, model.opt)
    ...
    if lang_eval == 1:
        lang_stats, imgToEval = language_eval(dataset, predictions, n_predictions, eval_kwargs, split, out_dir)

    chair_out = CHAIRevaluator.compute_chair_only(predictions)
    lang_stats['CHAIRs'] = chair_out['overall_metrics']['CHAIRs']
    lang_stats['CHAIRi'] = chair_out['overall_metrics']['CHAIRi']

    outfile_path = os.path.join(out_dir, model_id + '_' + split + '.json')
    with open(outfile_path, 'w') as outfile:
        json.dump({'overall': lang_stats, 'sentences': chair_out['sentences']}, outfile)
...
def eval_split_n(model, n_predictions, input_data, eval_kwargs={}):
    ...
    if sample_n_method == 'bs':
        ...
        for k in range(fc_feats.shape[0]):
            _sents = utils.decode_sequence(model.vocab, torch.stack([model.done_beams[k][_]['seq'] for _ in range(sample_n)]), model.opt)
            for sent in _sents:
    ...
    # case 2 sample / gumbel / topk sampling/ nucleus sampling
        ...
        for k in range(loader.batch_size):
            _sents = utils.decode_sequence(model.vocab, torch.stack([model.done_beams[k][_]['seq'] for _ in range(0, sample_n*beam_size, beam_size)]), model.opt)
            for sent in _sents:
                entry = {'image_id': data['infos'][k]['id'], 'caption': sent}
                n_predictions.append(entry)
    else:
        ...
        with torch.no_grad():
            _seq, _sampleLogprobs = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
        _sents = utils.decode_sequence(model.vocab, _seq, model.opt)
        for k, sent in enumerate(_sents):
