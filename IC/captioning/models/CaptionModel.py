...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, self.vocab_size).to(device) for _ in range(group_size)]
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if suppress_UNK and hasattr(self, 'vocab') and self.vocab[logprobs.size(1)-1] == 'UNK':
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.seq_length, bdash, self.vocab_size).zero_() for _ in range(group_size)]
    ...
    def decode_sequence(self, seq):
        return utils.decode_sequence(self.vocab, seq, self.opt)
