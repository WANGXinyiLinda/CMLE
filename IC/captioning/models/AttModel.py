...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.unk_idx = getattr(opt, 'unk_idx', self.vocab_size)

        self.use_bn = getattr(opt, 'use_bn', 0)
        self.use_hal = getattr(opt, 'use_hal', 0)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        outputs = fc_feats.new_zeros(batch_size, seq.size(1), self.vocab_size)
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains previous word indices
        xt = self.embed(it)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        if self.opt.use_hal:
            # first step, feed not_hal
            it = fc_feats.new_full([batch_size], self.opt.not_hal_idx, dtype=torch.long)
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            # second step, feed bos
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        # generate true captions (unhallucinated)
        ...
        for t in range(self.seq_length + start_t):
            if t == 0: 
                if self.opt.use_hal: # input <not_hal>
                    it = fc_feats.new_full([batch_size*sample_n], self.opt.not_hal_idx, dtype=torch.long)
                else: # input <bos>
                    it = fc_feats.new_full([batch_size*sample_n], self.bos_idx, dtype=torch.long)
            elif t == 1 and self.opt.use_hal: # input <bos>
                it = fc_feats.new_full([batch_size*sample_n], self.bos_idx, dtype=torch.long)
            ...
            if remove_bad_endings and t > start_t-1:
                tmp = logprobs.new_zeros(logprobs.size())
                prev_bad = np.isin(seq[:,t-start_t].data.cpu().numpy(), self.bad_endings_ix)
            ...
            if t == self.seq_length + start_t - 1: # skip if we achieve maximum length
                break
            elif t >= start_t - 1:
                ...
                seq[:,t - start_t + 1] = it
                seqLogprobs[:,t - start_t + 1] = logprobs
