...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, encoded_scr=None):
        "Take in and process masked src and target sequences."
        if not encoded_scr:
            encoded_scr = self.encode(src, src_mask)
        return self.decode(encoded_scr, src_mask, tgt, tgt_mask), encoded_scr
...
class TransformerModel(AttModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, use_ipm=False, att_masks=None, encoded_scr=None):
        if seq.ndim == 3:  # B * seq_per_img * seq_len
            seq = seq.reshape(-1, seq.shape[2])
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)

        out, encoded_scr = self.model(att_feats, seq, att_masks, seq_mask, encoded_scr)

        outputs = self.model.generator(out)

        if self.opt.use_hal:
            return outputs, encoded_scr
        else:
            return outputs
