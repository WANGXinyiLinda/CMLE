...
class TrainingArguments:
    ...
    master_port: int = field(
        default=29501, metadata={"help": "Master node (rank 0)'s free port that needs to "
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if self.no_cuda:
            ...
            torch.distributed.init_process_group(backend="nccl", init_method="tcp://localhost:{}".format(self.master_port))
            device = torch.device("cuda", self.local_rank)
            self._n_gpu = 1
