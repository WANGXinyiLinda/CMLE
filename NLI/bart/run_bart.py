...
def main():
    ...
    def load_data(file_str, revert, is_gen):
        ...
        for path in data_named_path:
            with open(registered_path[path]) as rf:
                for row in rf:
                    ...
                    if revert == 'h':
                        if is_gen:
                            for l in label_to_name:
                                if l != d['label'].strip():
                                    data['encode'].append(d['premise'])
                                    data['decode'].append('[' + l + ']' + 
                                        tokenizer.bos_token)
                        else:
                            data['encode'].append(d['premise'])
                            data['decode'].append('[' + d['label'].strip() + ']' + 
                                tokenizer.bos_token + d['hypothesis'])
                    elif revert == 'p':
                        if is_gen:
                            for l in label_to_name:
                                if l != d['label'].strip():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file is not None:
            datasets["train"] = load_data(data_args.train_file, data_args.revert, False)
        if data_args.validation_file is not None:
            datasets["validation"] = load_data(data_args.validation_file, data_args.revert, False)
        if data_args.test_file is not None:
            datasets["test"] = load_data(data_args.test_file, data_args.revert, True)
    ...
    #     return preprocess_function(examples, False, False)

    # def preprocess_function_gen(examples):
    #     return preprocess_function(examples, False, True)
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                output_test_preds_file = os.path.join(training_args.output_dir, 
                    "test_preds_{}.jsonl".format(data_args.test_file))
                with open(output_test_preds_file, "w") as writer:
                    # writer.write("\n".join(test_preds))
                    for prefix, pred in zip(datasets["test"], test_preds):
                        if data_args.revert == 'h':
                            example = {"premise": datasets["test"]["encode"],
                                "hypothesis": pred, 
                                "label":datasets["test"]["decode"][1]}
                        elif data_args.revert == 'p':
                            example = {"premise": pred,
                                "hypothesis": datasets["test"]["encode"], 
                                "label":datasets["test"]["decode"][1]}
